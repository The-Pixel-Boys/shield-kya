...
Antigravity is an agentic coding CLI (Google's Antigravity family) with no documented MCP server support, so there's nothing to wire a config into. KYA still gates it — through `kya wrap`, which evaluates a command before it runs and records the verdict on the trail.
...
Then put wrap in front of however you invoke Antigravity — a shell alias, a script, a CI step:

```bash
kya wrap --tool-id antigravity.run -- antigravity run "Refactor the auth module"
```
...
None. Wrap writes trail rows under `.kya/` in the project where you run it; Antigravity's own config is untouched.
...
Wrap gates the commands you route through it. Antigravity invocations made any other way — an interactive session, the IDE surface, an unwrapped script — never touch KYA. Coverage is exactly the set of invocations you wrap, no more.

## If Antigravity ships MCP support

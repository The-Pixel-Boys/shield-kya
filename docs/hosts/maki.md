...
Maki is an agent CLI host with no documented MCP server support, so there's nothing to wire a config into. KYA still gates it — through `kya wrap`, which evaluates a command before it runs and records the verdict on the trail.
...
Then put wrap in front of however you invoke Maki — a shell alias, a script, a cron job:

```bash
kya wrap --tool-id maki.run -- maki run "Summarize this diff"
```
...
None. Wrap writes trail rows under `.kya/` in the project where you run it; Maki's own config is untouched.
...
Wrap gates the commands you route through it. Maki invocations made any other way — an interactive session, an unwrapped script — never touch KYA. Coverage is exactly the set of invocations you wrap, no more.

## If Maki ships MCP support
